...
export default async function BukuBesarPage({ searchParams }: PageProps) {
  ...
  const pdfUrl = `/api/${activeOrg.id}/reports/buku-besar/pdf?startDate=${formatInputDate(startDate)}&endDate=${formatInputDate(endDate)}`
  const whatsappText = [
  ...
  return (
    <div className="max-w-full lg:max-w-7xl mx-auto px-4 sm:px-0 space-y-6">
      <div>
        <h1 className="text-2xl sm:text-3xl font-bold text-slate-800 tracking-tight">Buku Besar (General Ledger)</h1>
      ...
      <div id="report-content" className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        <div className="p-4 sm:p-6 border-b border-slate-100 bg-slate-50 flex items-center gap-3 flex-col sm:flex-row">
          ...
          <div>
            <h2 className="text-lg sm:text-xl font-bold text-slate-800">Buku Besar</h2>
            <p className="text-xs sm:text-sm text-slate-500">Rincian transaksi per akun untuk periode yang dipilih</p>
          </div>
        </div>
        <GeneralLedgerReport initialStartDate={formatInputDate(startDate)} initialEndDate={formatInputDate(endDate)} />
      </div>
    </div>
  )
}
