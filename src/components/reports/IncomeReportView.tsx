export default function IncomeReportView({ report }: { report: any }) {
  return (
    <div className="space-y-6">
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-slate-100">
            {report.data.map((trx: any, idx: number) => {
              const incomeLine = trx.reportBankLine
              ...
              return (
                <tr key={idx} className="hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3 text-right text-emerald-600 font-bold">Rp {(incomeLine?.debit || 0).toLocaleString('id-ID')}</td>
                </tr>
              )
            })}
          </tbody>
      ...
      {report.totals && (
        <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-4">
          <p className="text-sm text-slate-600 font-bold mb-1">Total Penerimaan</p>
          <p className="text-2xl font-bold text-emerald-600">Rp {report.totals.totalIncome.toLocaleString('id-ID')}</p>
        </div>
      )}
    </div>
  )
