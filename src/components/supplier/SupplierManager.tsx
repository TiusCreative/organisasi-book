...
import { X, Plus, Edit, Trash2, Phone, Mail, MapPin, Truck, Building } from "lucide-react"

export default function SupplierManager({ suppliers, organizationId }: any) {
  ...
  const handleEdit = (supplier: any) => {
    ...
    setFormData({
      ...
      paymentTerm: String(supplier.paymentTerm),
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-500">Kelola data supplier perusahaan</p>
        </div>
        <button
      ...
      {suppliers.length === 0 ? (
        <div className="text-center py-12 bg-slate-50 rounded-lg">
          ...
          <p className="text-slate-500">Belum ada supplier. Klik "Tambah Supplier" untuk menambahkan.</p>
        </div>
      ) : (
        <div className="grid gap-4">
          {suppliers.map((supplier: any) => (
            <div key={supplier.id} className="bg-white border rounded-lg p-5 hover:shadow-md transition">
              <div className="flex justify-between items-start">
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2">
                    <h3 className="font-semibold text-lg text-slate-800">{supplier.name}</h3>
                    <span className={`px-2 py-1 rounded text-xs font-medium ${supplier.status === 'ACTIVE' ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-600'}`}>
                  ...
                  {supplier.contactPerson && (
                    <p className="text-sm text-slate-600 mb-2">Kontak: {supplier.contactPerson}</p>
                  )}
                  ...
                  <div className="grid grid-cols-2 gap-2 text-sm text-slate-600">
                    ...
                    {supplier.mobile && (
                      <div className="flex items-center gap-2">
                        <Phone size={14} /> {supplier.mobile} (HP)
                      </div>
                    )}
                    {supplier.npwp && (
                      <div className="flex items-center gap-2">
                  ...
                  <div className="flex gap-4 mt-3 text-sm">
                    <span className="text-slate-500">Term: {supplier.paymentTerm} hari</span>
                  </div>
                </div>
